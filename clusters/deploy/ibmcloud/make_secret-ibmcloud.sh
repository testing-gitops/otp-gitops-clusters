...
if [[ -z ${IBMCLOUD_API_KEY} ]]; then
  echo "Please provide environment variable IBMCLOUD_API_KEY"
  exit 1
fi
...
cp templates/install-config.ibmcloud.yaml templates/install-config.yaml
...
ENC_IBMCLOUD_API_KEY=$(echo -n ${IBMCLOUD_API_KEY} | kubeseal --raw --name=$CLUSTER_NAME-ibmcloud-creds --namespace=$CLUSTER_NAME --controller-namespace $SEALED_SECRET_NAMESPACE --controller-name $SEALED_SECRET_CONTROLLER_NAME  --from-file=/dev/stdin)
...
sed -i '' -e 's#.*ibmcloud_api_key.*$#    ibmcloud_api_key: '$ENC_IBMCLOUD_API_KEY'#g' values.yaml
